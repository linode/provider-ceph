...
nilcheck: $(NILAWAY) ## Run nil check against codemake.
	...
	go list ./... | xargs -I {} -d '\n' $(NILAWAY) \
		-exclude-errors-in-files $(PWD)/cmd/provider/main.go,$(PWD)/internal/controller/bucket/bucket_backends.go,$(PWD)/internal/rgw/lifecycleconfig_helpers.go,$(PWD)/internal/rgw/objectlockconfiguration_helpers.go,$(PWD)/internal/rgw/serversideencryptionconfiguration_helpers.go,$(PWD)/internal/rgw/corsconfiguration_helpers.go \
